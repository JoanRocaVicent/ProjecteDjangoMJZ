...
const TOKEN_URL = 'http://192.168.18.238:8080/api/token/';   // ← change if your login endpoint differs
...
async function apiFetch(url, options = {}) {
  ...
  try {
    ...
    // With:
    const normalise = (item) => ({ ...item, id: item.id ?? item.pk });

    if (json.results) {
      // Paginated — fetch remaining pages if any
      let allResults = [...json.results];
      let nextUrl = json.next;
      while (nextUrl) {
        const nextRes = await fetch(nextUrl, { headers: { 'Authorization': `Bearer ${Auth.getToken()}` } });
        const nextJson = await nextRes.json();
        allResults = allResults.concat(nextJson.results || []);
        nextUrl = nextJson.next;
    }
    return { ok: true, data: allResults.map(normalise) };
  }
  return { ok: true, data: Array.isArray(json) ? json.map(normalise) : normalise(json) };
...
const BASE_URL = 'http://192.168.18.238:8080';
...
const API = {
  ...
  getRecursAutor(id)  { return apiFetch(`${BASE_URL}/autor/${id}/recursos/`)},
};
...
// Export resources of a specific author as JSON
async function exportAutorRecursosJSON(autorId) {
  const res = await API.getRecursAutor(autorId);
  ...
  a.href = url;
  a.download = `autor_${autorId}_recursos.json`;
  a.click();
...
// Export resources of a specific author as CSV
async function exportAutorRecursosCSV(autorId) {
  const res = await API.getRecursAutor(autorId);
  const autorRes = await API.getAutor(autorId); // fetch author once
  const autorName = autorRes.ok ? `${autorRes.data.nom} ${autorRes.data.cognoms}` : "";
  ...
  const data = res.data.map(r => ({
    ...
    data_publicacio: r.data_publicacio,
    is_activ: r.is_activ,
     autor: autorName
  }));

  if (!data.length) {
    showToast('Aquest autor no té recursos.', 'error');
    return;
  }
  ...
  const a = document.createElement("a");
  a.href = url;
  a.download = `autor_${autorId}_recursos.csv`;
  a.click();
    ...
    if (hash === '#/' || hash === '') {
      ...
      items.forEach((r, i) => {
        ...
        card.innerHTML = `
          ...
          ${r.autor ? `<p class="card-author">Autor: ${r.autor?.nom ?? ''} ${r.autor?.cognoms ?? ''}</p>` : ''}
          <div class="card-meta">
      ...
      box.innerHTML = `
        <div class="detail-card">
          ...
          <div class="meta-grid">
            <div class="meta-item"><label>Autor</label><span>${r.autor 
                                                                ? `<a href="#/autors/${r.autor.id}" style="color:var(--accent);text-decoration:none">
                                                                      ${r.autor?.nom ?? ''} ${r.autor?.cognoms ?? ''}
                                                                    </a>` 
                                                                : '—'}</span></div>
      ...
      const autorsRes = await API.getAutors();
      const autorSelect = document.getElementById('r-autor');

      if (autorsRes.ok && autorSelect) {
        autorsRes.data.forEach(a => {
          const opt = document.createElement('option');
          opt.value = a.id;
          opt.textContent = `${a.nom} ${a.cognoms}`;
      ...
      document.getElementById('recurs-form').addEventListener('submit', async (e) => {
        ...
        const data = {
          ...
          autor:      document.getElementById('r-autor').value || null
        };
      ...
      // attach export buttons for this author
      document.getElementById("export-autor-json").onclick =
        () => exportAutorRecursosJSON(id);

      document.getElementById("export-autor-csv").onclick =
        () => exportAutorRecursosCSV(id);
      ...
      // ── Load and render this author's linked resources ──────────
      const recursBox = document.getElementById('autor-recursos');
      const recursRes = await API.getRecursAutor(id);

      if (!recursRes.ok) {
        recursBox.innerHTML = `<p style="color:var(--accent);font-size:.9rem">${recursRes.error}</p>`;
        return;
      }

      const recursos = recursRes.data;
      const count = recursos.length;

      recursBox.innerHTML = `
        <h2 class="autor-recursos-title">
          Recursos
          <span class="autor-recursos-badge">${count}</span>
        </h2>`;

      if (!count) {
        recursBox.innerHTML += `<div class="empty" style="padding:1.5rem 0">
          <div class="empty-icon">📭</div>
          <p>Aquest autor no té cap recurs assignat.</p>
        </div>`;
        return;
      }
