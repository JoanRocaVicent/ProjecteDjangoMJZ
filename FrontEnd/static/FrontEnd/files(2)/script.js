...
const TOKEN_URL = '/api/token/';   // ← change if your login endpoint differs
...
// ── Public API object ─────────────────────────────────────────────
const API = {
  getRecursos()      { return apiFetch('/api/recursos/'); },
  getRecurs(id)      { return apiFetch(`/api/recursos/${id}/`); },
  createRecurs(data) { return apiFetch('/api/recursos/',        { method: 'POST',   body: JSON.stringify(data) }); },
  deleteRecurs(id)   { return apiFetch(`/api/recursos/${id}/`, { method: 'DELETE' }); },

  getAutors()        { return apiFetch('/api/autors/'); },
  getAutor(id)       { return apiFetch(`/api/autors/${id}/`); },
  createAutor(data)  { return apiFetch('/api/autors/',          { method: 'POST',   body: JSON.stringify(data) }); },
  deleteAutor(id)    { return apiFetch(`/api/autors/${id}/`,   { method: 'DELETE' }); },
};
